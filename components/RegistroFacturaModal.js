...
export default function RegistroFacturaModal({ show, onHide, id_solicitud, proveedor_id, onFacturaRegistered }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const resData = await fetch('/api/facturas-compras', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!resData.ok) {
        throw new Error(data.message || 'Error al registrar los datos de la factura.');
      }
    ...
    <>
      <Modal show={show} onHide={onHide} size="xl">
        <Modal.Header closeButton>
          <Modal.Title>Registrar Nueva Factura de Compra</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Container fluid>
