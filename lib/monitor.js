...
export function getSystemStats() {
  try {
    ...
    const solicitudes = db.prepare('SELECT COUNT(*) as count FROM solicitudes').get();
    const proveedores = db.prepare('SELECT COUNT(*) as count FROM proveedores').get();
    ...
    const solicitudesUrgentes = db.prepare(`
    ...
    // Usuarios activos (con logs en últimos 7 días)
    ...
    return {
      database: {
        size: dbStats.size,
        modified: dbStats.mtime.toISOString(),
        tables: {
